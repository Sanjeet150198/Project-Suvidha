...
class Controller extends Admin_Controller{
	...
	public function __construct(){
		...
		public function patient_existance($tab_name,$no){
			$stmt=$this->runQuery("select patient_id from $tab_name where no='".$no."'");
			if($stmt->rowCount()>0){
				return false;
				}
				else{
	...
	public function update($tab,array $col,$base_col){
		...
		//Fetch
		public function fetch_pat($tabname,array $col,$id){
			foreach($col as $row){
				$col[]=$row;
				}
			$cols=implode($col,",");	
			$result=$this->runQuery("select ".$cols." from ".$tabname." where patient_id='".$id."'");
			if($result->rowCount()>0){
					while($row=$result->fetch(PDO::FETCH_OBJ)){
						$data[]=$row;
						}
		...
		public function fetch_patient($table_name,$date){
		
		$stmt=$this->runQuery("select * from $table_name where appt_date='$date'");
		$data="";
		if($stmt->rowCount()>0){
		while($row=$stmt->fetch(PDO::FETCH_OBJ)){
			$data[]=$row;
